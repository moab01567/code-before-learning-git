...
class Start_up():
	...
	def login(self):
		try:
			...
			pass
		self.driver.find_element_by_xpath("//input[@id='login']").send_keys("19849")
		self.driver.implicitly_wait(10)
	...
	def sjekk_trade(self):
		...
		while True:
			...
			for i in range(len(info)):
				if i < 10:
					trade1.append(info[i])
					if len(trade1)== 10:
						rydd_info.append(trade1)
					else:pass

				else:
					trade2.append(info[i])
					if len(trade2) == 10:
						rydd_info.append(trade2)
					else:pass

			for tid, type_, lot, valuta, pip1, pip2, pip3, pip4, swap, profitt in rydd_info:
				resultat = profitt
				print(f"vi har en trade på {profitt}")
				if float(profitt) >= float(100.00):
					try:
						self.driver.find_elements_by_xpath("//span[@class='close']//span")[close[type_]].click()
						time.sleep(3)	
					except:
						print("problemer med å fjerne trade")
