...
class Start():
	...
	def hoved(self):
		while True:
			with open("/Users/mo/Desktop/Chart/chart2000/info2000.txt","r") as t:
				forhold = t.read()
			print(forhold)
			seconds = time.time()
			...
			try:
				if float(forhold) >= 1.0:
					print("stigende trend")
					trend_type = "buy"
					trend = True

				elif float(forhold) <= -1.0: 
...
class Start_up2():
	def __init__(self):
		...
		self.driver.get("https://trade.mql5.com/trade?servers=ICMarketsSC-Live16")
		time.sleep(1)
	...
	def login(self):
		try:
			...
			pass
		self.driver.find_element_by_xpath("//input[@id='login']").send_keys("1600100374")
		self.driver.implicitly_wait(10)
		...
		self.driver.implicitly_wait(10)
		self.driver.find_element_by_xpath("//option[@value='ICMarketsSC-Live16']").click()
		self.driver.implicitly_wait(10)
	...
	def fiks_oppsett(self):
		...
		fx_crosses = self.driver.find_elements_by_xpath("//div[@class='item first parent']//div[@class='item collapsed parent']")[7].click()
		time.sleep(1)
...
class kjop2():
	...
	def sell(self, pip2):
		...
		"""regn ut tp"""
		tp = int(pip_uten) - int(float(pip2))
